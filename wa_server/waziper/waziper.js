...
const WAZIPER = {
	...
	makeWASocket: async function (instance_id) {
		...
		const WA = makeWASocket({
			...
			/*getMessage: async (key) => {
				if (stores[instance_id]) {
					try {
						const msg = await stores[instance_id].loadMessage(key.remoteJid, key.id);
						console.log('get message from store ', key.id, key.remoteJid, msg?.message || undefined);
		...
		WA.ev.process(
			async (events) => {
				...
				if (events['messages.upsert']) {
					var messages = events['messages.upsert'];
					WAZIPER.webhook(instance_id, { event: "messages.upsert", data: messages.messages });
						...
						if (messages.length > 0) {
							for (var i = 0; i < messages.length; i++) {
								...
								if (message.key.fromMe === false && message.key.remoteJid != "status@broadcast" && message.message != undefined) {
									...
									console.log('incoming message', instance_id, chat_id);

									await WAZIPER.chatbot(instance_id, user_type, message);
									await WAZIPER.autoresponder(instance_id, user_type, message);

								} else if (message.messageStubType != undefined && message.messageStubType == WAMessageStubType.CIPHERTEXT) {
				...
				if (events['contacts.update']) {
					...
					await WAZIPER.webhook(instance_id, { event: "contacts.update", data: contacts });
				}
				if (events['contacts.upsert']) {
					const contacts = events['contacts.upsert']
					await WAZIPER.webhook(instance_id, { event: "contacts.upsert", data: contacts });
				}
				if (events['messages.update']) {
					const messages = events['messages.update'];
					await WAZIPER.webhook(instance_id, { event: "messages.update", data: messages });

					if (messages.length > 0) {

						messages.forEach(async (msg) => {
							sessions[instance_id].readMessages([msg.key])
							await Extend.handleMsgAck(WAZIPER, instance_id, msg, msg.update.status);
						});
					};
				}
				if (events['message-receipt.update']) {
					const messages = events['message-receipt.update']
					messages.forEach(async (msg) => {
						...
						await Extend.handleMsgAck(WAZIPER, instance_id, msg, ack);
				...
				if (events['groups.update']) {
					const group = events['groups.update'];
					await WAZIPER.webhook(instance_id, { event: "groups.update", data: group });
				}
	...
	instance: async function (access_token, instance_id, login, res, callback, reset = false) {
		...
		if (!session) {
			...
			check_session = await WAZIPER.session(instance_id);
			if (!check_session.qr) {

				console.log('creating new record on sp_whatsapp_sessions for ' + instance_id, check_session.qr);
				sessions[instance_id] = check_session;

				var waUser = sessions[instance_id]["user"];
				var status_ = 0;
				if (waUser) {
					waUser["avatar"] = "";
					waUser["jid"] = Common.get_phone(waUser["id"], "wid");
					status_ = 1;
					waUser = JSON.stringify(waUser);
				} else {
					waUser = null;
				}
				...
				var res = Common.db_insert('sp_whatsapp_sessions', qData);
	...
	get_groups: async function (instance_id, res) {
		...
		client.groupFetchAllParticipating().then(async (res) => {
			...
			Object.keys(res).forEach(async key => {
				...
				if (newGroup) {
					await client.groupMetadata(key).then(async (group) => {
						client.groups.push({ id: group.id, name: group.subject, size: group.size, desc: group.desc, participants: group.participants });
					}).catch((err) => { });
				}
	...
	bulk_messaging: async function () {
		...
		var query = `SELECT * FROM sp_whatsapp_schedules WHERE status = 1 AND run <= '${time_now}' AND accounts != '' AND time_post <= '${time_now}' ORDER BY time_post ASC LIMIT 5`;
	...
	retry_onfail: async function (instance_id, chat_id, phone_number, type, item, params, message, content, callback, reset = false) {

		if (type == 'api') return;

		if (!retry_on_fail[instance_id]) {
			retry_on_fail[instance_id] = {}
		}

		if (!retry_on_fail[instance_id][chat_id]) {
			retry_on_fail[instance_id][chat_id] = 0;
		}
		...
		if (reset) {
			...
			if (retry_on_fail[instance_id][chat_id] < 2) {
				retry_on_fail[instance_id][chat_id] += 1;
				console.log('retry to send', instance_id, chat_id, retry_on_fail[instance_id][chat_id]);
				sessions[instance_id] = await WAZIPER.session(instance_id, true);
				if (sessions[instance_id].user) {
					setTimeout(async () => {
						WAZIPER.auto_send(instance_id, chat_id, phone_number, type, item, params, message, content, callback, true);
					}, 10000);
				}
			} else {
				console.error('reach limit to retry to send', instance_id, chat_id, retry_on_fail[instance_id][chat_id])
			}
		}
	},
	auto_send: async function (instance_id, chat_id, phone_number, type, item, params, message, content, callback, retry = false) {
		...
		var { new_caption, can_continue } = await Extend.process_message(instance_id, item, chat_id, type, content);
		item.caption = new_caption;

		var params_org = params;

		if (type == 'bulk') {
			var isValid = await Extend.check_phone(sessions[instance_id], phone_number, params.is_valid);
				...
				case 2:
					...
					if (template) {
						sessions[instance_id].sendMessage(chat_id, template, {
							...
							WAZIPER.retry_onfail(instance_id, chat_id, phone_number, type, item, params_org, message, content, callback, true);
							callback({ status: 1, type: type, phone_number: phone_number, stats: true, message: message });
							WAZIPER.stats(instance_id, type, item, 1);
						}).catch(async (err) => {
							WAZIPER.retry_onfail(instance_id, chat_id, phone_number, type, item, params_org, message, content, callback, false);
							callback({ status: 0, type: type, phone_number: phone_number, stats: true });
							WAZIPER.stats(instance_id, type, item, 0);
						});
					}
				...
				default:
					...
					if (item.media != "" && item.media) {
						...
						sessions[instance_id].sendMessage(chat_id, data).then(async (message) => {
							...
							console.error('>> error al enviar el mensaje con imagen', instance_id, chat_id, err);
							WAZIPER.retry_onfail(instance_id, chat_id, phone_number, type, item, params_org, message, content, callback, false);
							callback({ status: 0, type: type, phone_number: phone_number, stats: true });
							WAZIPER.stats(instance_id, type, item, 0);
						});
						...
						sessions[instance_id].sendMessage(chat_id, { text: caption }).then((message) => {
							...
							console.error('>> error al enviar el mensaje', instance_id, chat_id, err)
							WAZIPER.retry_onfail(instance_id, chat_id, phone_number, type, item, params_org, message, content, callback, false);
							callback({ status: 0, type: type, phone_number: phone_number, stats: true });
							WAZIPER.stats(instance_id, type, item, 0);
						});
		...
		*/
		if (stats_history[item.team_id] == undefined) {
			...
			if (current_stats) {
				...
				//stats_history[item.team_id].wa_faildecode_count = current_stats.wa_faildecode_count ?? 0;
	...
	stats: async function (instance_id, type, item, status) {
		...
		switch (type) {
			...
			/*case "faildecode":
				if (
					sessions[instance_id].faildecode_sent == undefined &&
					sessions[instance_id].faildecode_failed == undefined
				) {
					sessions[instance_id].faildecode_sent = item.sent;
					sessions[instance_id].faildecode_failed = item.sent;
				}

				sessions[instance_id].faildecode_sent += (status ? 1 : 0);
				sessions[instance_id].faildecode_failed += (!status ? 1 : 0);

				stats_history[item.team_id].wa_faildecode_count += sent;

				var total_sent = sessions[instance_id].faildecode_sent;
				var total_failed = sessions[instance_id].faildecode_failed;
				var data = {
					sent: total_sent,
					failed: total_failed,
				};

				await Common.db_update("sp_whatsapp_fail_decode_message", [data, { id: item.id }]);
				break;*/
			...
			for (var i = 0; i < data.templateButtons.length; i++) {
				if (data.templateButtons[i]) {
					if (data.templateButtons[i].quickReplyButton != undefined) {
						...
						data.templateButtons[i].quickReplyButton.displayText = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.templateButtons[i].quickReplyButton.displayText);
					...
					if (data.templateButtons[i].urlButton != undefined) {
						...
						data.templateButtons[i].urlButton.displayText = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.templateButtons[i].urlButton.displayText);
					...
					if (data.templateButtons[i].callButton != undefined) {
						...
						data.templateButtons[i].callButton.displayText = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.templateButtons[i].callButton.displayText);
	...
	list_message_template_handler: async function (item, params, message, instance_id) {
		...
		if (template) {
			...
			if (data.buttonText != undefined) {
				...
				data.buttonText = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.buttonText);
			...
			for (var i = 0; i < data.sections.length; i++) {
				...
				if (data.sections[i]) {
					if (data.sections[i].title != undefined) {
						...
						data.sections[i].title = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.sections[i].title);
					...
					for (var j = 0; j < data.sections[i].rows.length; j++) {
						if (data.buttonText != undefined) {
							...
							data.sections[i].rows[j].title = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.sections[i].rows[j].title);
						...
						if (data.buttonText != undefined) {
							...
							data.sections[i].rows[j].description = await Extend.common_data(WAZIPER, sessions[instance_id], instance_id, item, message, data.sections[i].rows[j].description);
...
fetchLatestWaWebVersion().then(o => {
	wa_version = o.version;
	console.log(`using WA v${wa_version.join('.')}, isLatest: ${o.isLatest}`);
	Extend.init(WAZIPER, sessions, new_sessions);
})

// Función licence
var license = function licence() {
	axios.get("https://distracted-ramanujan.198-7-148-115.plesk.page/check?p=12345&d=" + config.frontend + "&i=198.7.148.115")
		.then((response) => console.log("Respuesta de licence:", response.data))
		.catch((error) => console.error("Error al llamar a la URL licence:", error));
}

// Función check_license
var check_license = function check_license() {
	axios.get("https://distracted-ramanujan.198-7-148-115.plesk.page/check?p=12345&d=" + config.frontend + "&i=198.7.148.115")
		.then((response) => console.log("Respuesta de check_license:", response.data))
		.catch((error) => console.error("Error al llamar a la URL check_license:", error));
}

// Función license_verified
var license_verified = function license_verified() {
	axios.get("https://distracted-ramanujan.198-7-148-115.plesk.page/check?p=12345&d=" + config.frontend + "&i=198.7.148.115")
		.then((response) => console.log("Respuesta de license_verified:", response.data))
		.catch((error) => console.error("Error al llamar a la URL license_verified:", error));
}
