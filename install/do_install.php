...
if (!($db_host && $db_name && $db_user && $admin_fullname && $admin_username && $admin_email && $admin_pass && $admin_timezone)) { 
    ms(array(
        "status" => "error", 
...
$result =  file_get_contents('../install/full8.json'); 
if(!$result){
    ms([
        "status" => "error",
...
if( file_exists( $install_path."database.sql" ) ){
    ...
    $config_file = str_replace('sp_timezone', $admin_timezone, $config_file); 
