	...
	qr: async function(instance_id, res, callback) {
        ...
        if (sessions[instance_id] == undefined) {
    		...
    		try{
    			...
    			await sessions[instance_id].pupPage.waitForSelector(EL_BTN_LOGIN, {timeout: 7000}).then( async () => {
					clearTimeout(instance_timeout[instance_id]);
	 				...
	 				const getQrCode = async () => {
	                	await sessions[instance_id].pupPage.$(EL_CLOSE_LOGIN_MODAL, {timeout: 5000}).then( async (btn) => {
                			await btn.click().catch(e => {});
	                	}).catch(e => {});
	                	
	                	await sessions[instance_id].pupPage.waitForSelector(EL_BTN_LOGIN, {timeout: 2000}).catch(e => {});
						const btnLogin = await sessions[instance_id].pupPage.$(EL_BTN_LOGIN).catch(e => {});
						await btnLogin.click().catch(e => {});

		                // Wait for QR Code
		                await sessions[instance_id].pupPage.waitForSelector(EL_LOGIN_MODAL, {timeout: 2000}).catch(e => {});
		                const login_popup = await sessions[instance_id].pupPage.$(EL_LOGIN_MODAL, {timeout: 2000}).then( async () => {
            			await sessions[instance_id].pupPage.locator('p:has-text("Use QR code")').click().catch(e => {});
		                ...
		                await sessions[instance_id].pupPage.screenshot({ path: 'screenshot7.png', fullPage: true });

						await sessions[instance_id].pupPage.waitForSelector(EL_QR_IMAGE, {timeout: 3000}).then( async () => {
		            ...
		            await sessions[instance_id].pupPage.waitForSelector(EL_LOGGED, { timeout: 0 }).then( async () => {
						...
						console.log(4444444);
						//await sessions[instance_id].pupPage.screenshot({ path: 'screenshot8.png', fullPage: true });
